...
Vector3 Gravity::CalculateGravity()
{
    ...
    for (auto element : GetTransform()->GetObjects())
    {
        ...
        float gravity_x = k_G * (GetInverseMass() * element->GetPhysics()->GetMass()) / (distance.x * distance.x);
        float gravity_y = k_G * (GetInverseMass() * element->GetPhysics()->GetMass()) / (distance.y * distance.y);
        float gravity_z = k_G * (GetInverseMass() * element->GetPhysics()->GetMass()) / (distance.z * distance.z);
