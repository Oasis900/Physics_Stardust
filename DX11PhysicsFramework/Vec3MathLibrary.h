...
// Constructor
inline Vector3::Vector3(const float xi, const float yi, const float zi)
{
    x = xi;
    y = yi;
    z = zi;
...
{
    float m = static_cast<float>(sqrt(x * x + y * y + z * z));
...
inline Vector3 operator*(const float s, const Vector3 u)
{
    return Vector3(u.x * s, u.y * s, u.z * s);
}


inline Vector3 operator*(const Vector3 u, const float s)
{
    return Vector3(u.x * s, u.y * s, u.z * s);
}
...
inline float TripleScalarProduct(const Vector3 u, const Vector3 v, const Vector3 w)
{
